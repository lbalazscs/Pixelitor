...
public class PlafPaintUtils {
     ...
     */
    static synchronized Paint getVerticalGradient(String name,
                                                  int height, int y,
                                                  float[] positions,
                                                  Color[] colors) {
        if (verticalGradients == null) {
            verticalGradients = new HashMap<>();
        }
        ...
        if (paint == null) {
            ...
            BufferedImage bi = new BufferedImage(1, height, TYPE_INT_ARGB);
    ...
    public static TexturePaint getCheckerBoard(int checkerSize, Color color1, Color color2) {
        String key = checkerSize + " " + color1 + " " + color2;
        if (checkers == null) {
            checkers = new HashMap<>();
        }
        TexturePaint paint = checkers.get(key);
        if (paint == null) {
            BufferedImage bi = new BufferedImage(2 * checkerSize, 2 * checkerSize, TYPE_INT_RGB);
