...
public class MeasuredShape implements Serializable {
    ...
    static class Segment implements Serializable {
        ...
        private static float calculateDistance(double ax, double bx, double cx, double dx,
    ...
    public MeasuredShape(PathIterator i, float spacing) {
        ...
        if (!v.isEmpty()) {
            ...
            if (Math.abs(last.data[last.data.length - 2] - moveX) > 0.001 ||
                ...
                coords[0] = moveX;
                coords[1] = moveY;
                Segment s = new Segment(SEG_LINETO, lastX, lastY, coords, spacing);
                v.add(s);
                closedDistance += s.realDistance;
            }
        }
    ...
    public void writeShape(float position, float length, PathWriter w, boolean includeMoveTo) {
        if (length >= 0.999999f) {
            writeShape(w);
            return;
        } else if (length <= -0.999999f) {
            writeShapeBackwards(w);
            return;
        } else if (length < 0.000001 && length > -0.000001) {
            return;
        }
    ...
    public Point2D getPoint(float distance, Point2D dest) {
        if (distance < 0) {
            throw new IllegalArgumentException("distance (" + distance + ") must not be negative");
        }
        if (distance > closedDistance) {
            throw new IllegalArgumentException("distance (" + distance + ") must not be greater than the total distance of this shape (" + closedDistance + ")");
        }
        if (dest == null) {
        ...
        dest.setLocation(segments[0].getX(0), segments[0]
                .getY(0)); //a fluke case, where we're basically at the end of the shape
        return dest;
    }
    ...
    public float getCommonDistance(MeasuredShape s) {
        ...
        for (int a = 0; a < m; a++) {
            if (segments[a].type != SEG_MOVETO &&
                    s.segments[a].type != SEG_MOVETO) {
            ...
            } else if (segments[a].type == SEG_MOVETO &&
                    s.segments[a].type == SEG_MOVETO) {
                //skip
            } else {
                return distance;
