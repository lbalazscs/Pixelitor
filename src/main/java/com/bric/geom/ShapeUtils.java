...
public class ShapeUtils {
    /**
     * Return false if any coordinates in this shape
    ...
    public static int getSubPathCount(Shape s) {
        ...
        float[] coords = new float[6];
        while (!i.isDone()) {
            if (i.currentSegment(coords) == SEG_MOVETO) {
                ctr++;
            }
            i.next();
        }
    ...
    public static boolean equals(Shape shape, Shape shape2) {
        ...
        while (!iter1.isDone() && !iter2.isDone()) {
            ...
            if (k1 == SEG_MOVETO || k1 == SEG_LINETO) {
                if (!equals(coords1, coords2, 2)) {
                    return false;
                }
            } else if (k1 == SEG_QUADTO) {
                if (!equals(coords1, coords2, 4)) {
                    return false;
                }
            } else if (k1 == SEG_CUBICTO) {
                if (!equals(coords1, coords2, 6)) {
                    return false;
                }
            } else if (k1 == SEG_CLOSE) {
                //do nothing
            } else {
