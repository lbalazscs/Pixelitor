...
public abstract class Clipper {
    ...
    static class ClippedPath {
         ...
         */
        public void flush() {
            while (!uncommittedPoints.isEmpty()) {
    ...
    public static void clip(Graphics2D g, Shape newClip) {
        ...
        if (newRect != null && oldRect == null) {
            GeneralPath intersectedClip = clipToRect(oldClip, newRect);
            g.setClip(intersectedClip);
            return;
        }

        if (newRect == null && oldRect != null) {
            GeneralPath intersectedClip = clipToRect(newClip, oldRect);
            g.setClip(intersectedClip);
            return;
        }
