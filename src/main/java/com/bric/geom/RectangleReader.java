...
public class RectangleReader {
     ...
     * or <code>null</code>.
     */
    public static Rectangle2D convert(Shape shape) {
     ...
     * or <code>null</code>.
     */
    public static Rectangle2D convert(Shape shape, AffineTransform transform) {
        if (shape == null) {
            return null;
        }
     ...
     */
    private static Rectangle2D getRectangle(Rectangle2D r) {
        double x = r.getX();
        double y = r.getY();
        double w = r.getWidth();
     ...
     * the null if not.
     */
    private static Rectangle getRectangle(double x, double y, double w, double h) {
        if (w < 0) {
            x = x + w;
            w = -w;
        }
