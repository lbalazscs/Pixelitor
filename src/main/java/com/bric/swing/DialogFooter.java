...
public class DialogFooter extends JPanel {
    ...
    private static final KeyStroke commandPeriodKey = KeyStroke
            .getKeyStroke(KeyEvent.VK_PERIOD, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx());

    /**
     * The localized strings used in dialogs.
     */
    private static final ResourceBundle strings = ResourceBundle.getBundle("com.bric.swing.resources.DialogFooter");

    /**
     * This is the client property of buttons created in static methods by this class.
     */
    public static final String PROPERTY_OPTION = "DialogFooter.propertyOption";
    ...
    private static final AncestorListener escapeTriggerListener = new AncestorListener() {
        ...
        private void setRootPaneContainer(JButton button, RootPaneContainer c) {
            ...
            if (lastContainer != null) {
                lastContainer.getRootPane().getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                        .remove(escapeKey);
    ...
    public static JButton createDontSaveButton(boolean escapeKeyIsTrigger) {
        ...
        button.putClientProperty(DialogFooter.PROPERTY_META_SHORTCUT, text.charAt(0));
        if (escapeKeyIsTrigger) {
            makeEscapeKeyActivate(button);
        }
        return button;
    ...
    public static DialogFooter createDialogFooter(int options, EscapeKeyBehavior escapeKeyBehavior) {
        return createDialogFooter(EMPTY_ARRAY, options, escapeKeyBehavior);
    }
     ...
     */
    public static final boolean reverseButtonOrder = JVM.isMac;

    protected final JComponent[] leftControls;
    protected final JComponent[] dismissControls;
    ...
    private void innerAction(ActionEvent e) {
                ...
                if (dismissControl instanceof JButton) {
                    ...
                    if (ch != null) {
                        KeyStroke keyStroke = KeyStroke
                                .getKeyStroke(ch.charValue(), Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx());
    ...
    public DialogFooter(JComponent[] leftControls, JComponent[] dismissControls, boolean autoClose, JButton defaultButton) {
        ...
        for (int a = 0; a < dismissControls.length; a++) {
            ...
            if (dismissControls[a] instanceof JButton) {
                ((JButton) dismissControls[a]).addActionListener(this::innerAction);
            } else {
                ...
                try {
                    ...
                    m.invoke(dismissControls[a], (ActionListener) this::innerAction);
                } catch (Throwable t) {
                    //do nothing
                }
            }
    ...
    private void installGUI() {
        ...
        int unsafeCtr = 0;
        int safeCtr = 0;
        for (JComponent control : dismissControls) {
            if (JVM.isMac && isUnsafe(control)) {
                unsafeCtr++;
            } else {
                safeCtr++;
            }
