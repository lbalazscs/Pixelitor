 ...
 * original image with increasing opacity.)
 */
public class MicroscopeTransition2D extends Transition2D {
    ...
    public Transition2DInstruction[] getInstructions(float progress,
                                                     Dimension size) {
        progress = (progress * progress * progress) * 6.0f / 7.0f + progress / 7.0f;
        ...
        for (int a = 0; a < r.length; a++) {
            float multiplier = (float) Point2D
                    .distance(size.getWidth() / 2, size.getHeight() / 2, r[a].getCenterX(), r[a].getCenterY());
