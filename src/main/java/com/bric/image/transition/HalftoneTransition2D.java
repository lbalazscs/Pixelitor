...
public class HalftoneTransition2D extends Transition2D {
    ...
    public Transition2DInstruction[] getInstructions(float progress,
        ...
        float ySize = size.height * 0.05f;
        float xSize = size.width * 0.05f;
        List<RectangularShape> v = new ArrayList<>();
        float angleProgress = (float) Math.pow(1 - Math.min(progress, 1), 0.5);
        //pop it over 1:
