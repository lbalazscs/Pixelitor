...
public class WaveTransition2D extends Transition2D {
    ...
    public Transition2DInstruction[] getInstructions(float progress, Dimension size) {
        ...
        for (int a = 0; a < i.length; a++) {
            ...
            float wave = (float) (0.3 * sin(1 + 3 * z1 + 8 * progress));
            float dip = 1 - 0.5f * (2 * progress - 1) * (2 * progress - 1) * (2 * progress - 1) - 0.5f;


            double d = (1 - progress2) * measurement + (1 - z2) * dip * (z2 + wave + 0.4) * measurement;
