...
public class DotsTransition2D extends AbstractClippedTransition2D {
    private final Ellipse2D[] bubbles = {
            make(0.680542285974303, 0.18889704343632807, 0.19951594932207756),
            ...
            make(0.4349481618226607, 0.25633754562508043, 0.1165154509890553),
            make(0.9809361717371541, 0.30209191539494906, 0.1333244867709235),
            ...
            make(0.2543162939332376, 0.16250491418006419, 0.13575009482043435),
            ...
            make(0.9464285339584122, 0.034394073749900445, 0.21788008758584235)
    };
    ...
    protected static Ellipse2D.Double make(double x, double y, double r) {
    ...
    protected Shape[] getShapes(float progress, Dimension size) {
        ...
        for (int a = 0; a < bubbles.length; a++) {
            float k = a / (bubbles.length - 1.0f);
            float base = (float) (Math.sqrt(k) * 0.25 + k * 0.75) * domain;
    ...
    public float getStrokeWidth(float progress, Dimension size) {
        float f = Math.max(size.width - 300.0f, 0);
        f = f / 700.0f + 0.3f;
